	...
	\pgsgd{\textbf{(}$\mathcal{G}$\textbf{)}}{
		...
		\for{$\eta$ $in$ $annealing$ $schedule$}{ %our "schedule" actually is the number paths.... we should specify this I would say
			\each{$planned$ $term$ $update$} { % I think we can remove i<j, because we don't care about that
				...
				$\delta \gets \mu \cdot \frac{ld_{ij} -nd_{ij}}{2}$ \tcp{the actual delta}
				\uIf{abs($\delta) <= 0$} {
					$STOP$ \tcp{we can't optimize more}
				}
				% TODO stop early?
				% TODO potentially store new delta max?
				$r \gets \frac{\delta}{ld_{ij}}$ \tcp{size of the update}
				$r_x \gets r \cdot (l_{i} - l_{j})$ \tcp{update size normalized by layout distance}
				$l_i \gets l_i + r_{x}$ \tcp{update $v_i$ coordinates}
				$l_j \gets l_j - r_{x}$ \tcp{update $v_j$ coordinates}
			}
		}
	}
	\caption{Pseudocode of PG-SGD in 1D.}
	\label{alg:pg_sgd}
\end{algorithm}