...
\begin{algorithm}
	\pgsgd{\textbf{(}$\mathcal{V}$\textbf{)}}{
		...
		\for{$planned$ $term$ $updates$}{ %our "schedule" actually is the number paths.... we should specify this I would say
			\each{$\eta$ $in$ $annealing$ $schedule$} { % I think we can remove i<j, because we don't care about that
