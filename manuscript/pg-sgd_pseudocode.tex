...
\begin{algorithm}
	\pgsgd{\textbf{(}$\mathcal{G}$\textbf{)}}{
		...
		\for{$\eta$ $in$ $annealing$ $schedule$}{ %our "schedule" actually is the number paths.... we should specify this I would say
			\each{$planned$ $term$ $update$} { % I think we can remove i<j, because we don't care about that
				...
				\uIf{$\delta <= 0$} {
					$STOP$ \tcp{we can't optimize it more}
				}
				% TODO stop early?
	...
	\caption{Pseudocode of the PG-SGD algorithm. \FIXME{Explain $\eta$. Explain $PathIndex$ in the text. Explain $InitZipf$ in the text. Explain $RandomLayout$ in the text. Explain annealing schedule in the text or rephrase this wording.}}
	\label{alg:pg_sgd}
\end{algorithm}