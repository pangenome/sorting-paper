...
\begin{document}
	...
	It's mathematical formulation is called the Zipfian distribution and it rank-frequency distribution is related to the inverse power law.
	...
	his random placement ensures that each run of the PG-SGD method could potentially yield a different result, thereby increasing the likelihood of obtaining a meaningful layout.
	...
	Our implementation is multithreaded and follows the HOGWILD! strategy \citep{Recht2011}, that is threads perform layout updates without any locking.
	This is possible because pangenome graphs are tipically sparse \citep{Guarracino2022}, with low average node degree, meaning most updates only modify small parts of the entire layout.
	With respect to the HOGWILD! SGD algorithm, which writes the layout updates to a shared non-atomic integer vector, PG-SGD stores node coordinates in an atomic integer vector.
	We prefer an atomic vector to ensure memory safety because, in our experience, there is no performance loss with respect to the non-atomic counterpart.
	...
	Moreover, we are considering extending the algorithm to RNA and protein sequences to support pantranscriptomes \citep{sibbesen_haplotype-aware_2023} and panproteomes \citep{dabbaghie_panpa:_2023}, respectively.
