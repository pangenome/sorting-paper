...
\begin{document}
	...
	The PG-SGD algorithm is inspired by \cite{Zheng2019}, but we designed it to work on pangenomes in the form of variation graph (\ref{def:vg}).
...
\end{definition}
	...
	The algorithm moves a single pair of nodes at a time, minimizing the disparity between the layout distance of a node pair and the nucleotide distance of the same nodes as calculated along a path that traverses them.
	Our path index enables efficient retrieval of path positions to quickly compute nucleotide distances.
	This eliminates the need to store all pairwise distances between nodes in memory, allowing the PG-SGD to be applied to gigabase-scale pangenome graphs. \\
	...
	Our implementation is multithreaded, but for the sake of simplicity, we describe how the algorithm works on a single thread (~\ref{alg:pg_sgd}).
	...
	\FIXME{Be brief and specify that it works because the pangenome graphs are sparse}
	\subsection{Layout initialization}
	\FIXME{1D: random, node order in the input graph; 2D: Random (uniform or gaussian), hilbert}
	\subsection{Zipfian distribution}
	\FIXME{Describe what it is and why we use it (SPOILER: it just works)}
	\subsection{Annealing}
	\FIXME{Describe the problem (noise aat local level) and how we solve it by 'annealing', that is slowing down the movements}
	\subsection{Stop criteria, iterations, term updates}
	\FIXME{Describe briefly}
	\subsection{Cooling and flip}
	\FIXME{Describe briefly}
	\subsection{From 1D layout to 1D node order}
	\FIXME{Not sure, so very short}
