...
\begin{document}
	...
	Experiments were conducted on a cluster with 24 \texttt{Regular} nodes (32 cores / 64 threads 2* AMD EPYC 7343 processors with 512 GB RAM each) and 4 \texttt{HighMem} nodes (64 cores / 128 threads 2* AMD EPYC 7513 processors with 2048 GB RAM each). The high-mem nodes were only used for the calculations of the 2D layouts of the full HPRC graph including all chromosomes.
	...
	When applied to these pangenome graphs using one \texttt{Regular} node for each calculation, PG-SGD obtains the graph layouts in \FIXME{XXXX minutes/hours on average}, with the slowest chromosome being chromosome 16.
	...
	Memory consumption is \FIXME{XXXX GB of RAM on average}, with the memory peak occurring with chromosome 1. An overview of these evaluations is given in Supplementary Table~\ref{tab:layout}.
